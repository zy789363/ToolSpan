...
const ownerPasswordSchema = z.string().refine(
  (value) => getPasswordLength(value) >= MIN_OWNER_PASSWORD_LENGTH,
...
const formSchema = z.object({
  ...
  ownerPasswordConfirm: ownerPasswordSchema,
    ...
    <main className="onboarding-shell">
      ...
      <Card className="onboarding-card">
        ...
        <div className="onboarding-body">
          ...
          {step === 5 ? <div className="password-fields"><div className="password-guidance" id="owner-password-requirements"><strong>{t("onboarding.passwordRequirements")}</strong><ul><li className={getPasswordLength(passwordValue) >= MIN_OWNER_PASSWORD_LENGTH ? "is-met" : ""}><Check aria-hidden="true" size={14} />{t("onboarding.passwordRequirementLength")}</li><li><ShieldCheck aria-hidden="true" size={14} />{t("onboarding.passwordRequirementStrength")}</li></ul></div><label className="field"><span>{t("onboarding.passwordLabel")}</span><input aria-describedby="owner-password-requirements" aria-invalid={errors.ownerPassword === undefined ? undefined : true} autoComplete="new-password" autoFocus minLength={MIN_OWNER_PASSWORD_LENGTH} placeholder={t("onboarding.passwordPlaceholder")} type="password" {...register("ownerPassword")} />{errors.ownerPassword === undefined ? null : <small role="alert">{t("onboarding.validationPassword")}</small>}</label><label className="field"><span>{t("onboarding.passwordConfirmLabel")}</span><input aria-describedby="owner-password-requirements" aria-invalid={errors.ownerPasswordConfirm === undefined ? undefined : true} autoComplete="new-password" minLength={MIN_OWNER_PASSWORD_LENGTH} placeholder={t("onboarding.passwordPlaceholder")} type="password" {...register("ownerPasswordConfirm")} />{errors.ownerPasswordConfirm === undefined ? null : <small role="alert">{errors.ownerPasswordConfirm.message === "password_mismatch" ? t("onboarding.validationPasswordMatch") : t("onboarding.validationPassword")}</small>}</label><PasswordStrength value={passwordValue} /></div> : null}
