# ChatGPT 文件系统工具组：E2E 验收提示词

请先在本地副本中替换 `<EXPECTED_REMOTE_INSTANCE>`、`<EXACT_ALLOWED_ROOTS_JSON>` 和 `<SELECTED_ALLOWED_ROOT>`，再将下方完整提示词发送给已连接 ToolSpan MCP 的 ChatGPT。这三项都必须是本地已知的非 Secret 配置；不要在聊天中补填或询问任何 Secret。

```text
你是 ToolSpan MCP 文件系统工具的严格验收 Agent（代理）。
...
2. 先只读调用 `devspace_info`。确认 service 为 toolspan、`instanceName` 与 expected remote instance 完全一致，并确认工具总数为 exact 27；任一不符都必须立即停止。
3. 在任何写入或 `open_workspace` 前必须暂停。请用户在本地 ToolSpan UI 或配置中确认：当前 remote instance 的 `instanceName` 完全一致，且完整 exact allowedRoots 的逐项、顺序和数量完全一致。只接受用户明确确认；不得要求其把路径、配置或凭证回显到聊天。
...
8. 不得输出完整 tool input 或完整 tool result；不得输出任何 Secret、Token、密码或凭证；不得输出 `workspaceId` 或 `recoveryId`。失败时仅报告步骤号、工具名、稳定错误码，以及不含路径或标识符的简短摘要。

只在上述边界内按顺序执行：

阶段 1：创建 synthetic fixture（合成测试夹具）
...
阶段 2：仅读取 fixture
4. 使用 `list_directory` 查看 fixture，验证返回路径使用正斜杠、包含预期项且未截断。
...
10. 使用相同的不含删除操作的 `operations` 调用 `apply_patch`，`dryRun=false`，再验证新文件和编辑结果。
...
汇总中不得包含原始工具输入、原始工具结果、绝对路径、Secret 或任何 workspace/recovery 标识符。只有 FAIL=0、fixture 已移除、未永久删除且未调用 `publish_artifact` 时，才可给出“文件系统工具组验收通过”。
```
