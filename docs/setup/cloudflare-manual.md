# Cloudflare Guided Manual

这条路径不要求把 Cloudflare 管理凭证交给 ToolSpan。用户在 Cloudflare Dashboard 与本机官方 `cloudflared` 流程中操作，ToolSpan 只展示非秘密状态、URL 与检查结果。示例目标固定为 `aiqushi.top` / `mcp.aiqushi.top`；若使用其他域名，应明确重新选择，而不是让 Agent自行替换。

每一步都包含目的、操作、预期结果、失败分支和回滚/恢复。任何一步没有得到预期结果都先停止，不跳过门禁。

## 1. Local health

- **目的：** 证明 Headless Core 在 loopback 上可用，且本地端口与当前实例一致。
- **操作：** 启动 ToolSpan；在 Desktop 中读取本地 endpoint，或对 `http://127.0.0.1:<port>/healthz` 做 GET。用 Desktop Host 的已授权状态确认 `instanceName`，不要从公开 `/healthz` 期待实例名。
- **预期结果：** HTTP 200；响应只含 `service`、`status`、`version`，`status` 为 `ok`。本地 MCP endpoint 为同一 loopback origin 的 `/mcp`。
...
- **目的：** 证明目标域名已在正确 Cloudflare account 中，并且 Zone 真正可用于代理流量。
- **操作：** 在 Dashboard 选择 `aiqushi.top`，记录非秘密的 account ID、zone ID、zone status 与 Cloudflare assigned nameservers；检查 `mcp.aiqushi.top` 是否已有 DNS 记录。
- **预期结果：** Zone 存在且 status 为 `Active`；未知来源的同名 DNS 记录为 0，或已明确标为冲突并停止。
- **失败分支：** Zone 缺失或 `Pending Nameserver Update` 时禁止 Apply，转到 [Zone onboarding](cloudflare-zone-onboarding.md)。状态为 Moved/Deleted/Initializing 时也停止并按 Cloudflare 官方说明处理。
- **回滚/恢复：** 本步骤只读，外部副作用为 0。不删除其他 account/zone，不修改 registrar credential。

## 3. Create or select Tunnel

- **目的：** 得到一个专用于当前 ToolSpan instance 的 Named Tunnel，同时避免撞到外部资源。
- **操作：** 在 Cloudflare Dashboard 创建以 `toolspan-` 为前缀、可识别 instance 的 Tunnel；若同名 Tunnel 已存在，先核对 receipt/journal 中的 ID 和 ownership，无法证明则视为 external collision。
- **预期结果：** 新建 Tunnel 被分类为 `created`，或由既有 ToolSpan receipt 证明的 Tunnel 被分类为 `reused`；记录 Tunnel ID/name，但不记录任何运行 token。
- **失败分支：** 同名但 ID/ownership 不符、API/UI 返回冲突、账号不匹配时停止，不自动删除或改名重试。
- **回滚/恢复：** 只有本 session 的创建响应和 journal 同时证明 `created` 才可在已确认的 rollback 中删除；`reused` 永不自动删除。

## 4. Route hostname to local Core

- **目的：** 将 `mcp.aiqushi.top` 的 Tunnel ingress 指向 `http://127.0.0.1:<port>`，并避免无意暴露其他本地服务。
...
- **预期结果：** ingress 只含所需 hostname → loopback ToolSpan service，以及安全 catch-all；pre-change 非秘密 fingerprint 已记录。
- **失败分支：** 现有 ingress 来源未知、指向不同端口/服务、或缺少可验证的 precondition 时停止；不要覆盖整个配置。
- **回滚/恢复：** 仅在 fingerprint 仍匹配时恢复本 session 修改前的 ingress；不匹配时进入 `ROLLBACK_PARTIAL` 并给出手工步骤。
...
## 6. Install / run cloudflared

- **目的：** 用 Cloudflare 官方 runtime 把 Named Tunnel 连接到本机 ToolSpan。
- **操作：** 从 Cloudflare 官方来源核对 `cloudflared` 版本；选择 foreground test 或 service。需要管理员权限时显示 UAC checkpoint 并暂停。运行凭证只进入官方 `cloudflared` credential/service storage，不粘贴到 ToolSpan。
- **预期结果：** foreground 连接健康，或 service 的创建/ownership/运行状态被明确验证；ToolSpan config、DB、journal、日志、Prompt 与 receipt 中运行凭证数量为 0。
- **失败分支：** 无管理员权限时输出手工步骤，不声称 service installed；已有外部 service 时不接管；错误日志必须 redacted。
...
## 9. Host scan and exact 27 tools

- **目的：** 证明协议初始化、OAuth、工具发现及 exact 27 Tool Contract 在真实 Host 上成立。
- **操作：** 先用 MCP Inspector 对公开 `/mcp` 初始化、列出并抽样调用工具；再在实际可用 Host 添加连接。写操作前调用 `devspace_info` 确认 `instanceName`、workspace 与 allowed roots。
- **预期结果：** 工具总数恰好 27，名称/输入 schema 与冻结 fixture 一致；只读验证与受控写验证分别留存非秘密证据。
- **失败分支：** 工具不是 27、OAuth consent 未完成、Host 套餐/策略限制或 write 不可用时不改契约。ChatGPT 受限记 `BLOCKED_BY_HOST_PLAN_OR_POLICY`；Codex 承担真实 write/job gate。
- **回滚/恢复：** 撤销测试 OAuth grant、清理由验证创建且 ownership 可证明的 synthetic workspace 资源；保留非秘密 evidence，不删除用户数据。

## 完成证据

Guided Manual 完成不等于外部 Gate 自动 PASS。记录真实命令/页面、时间、非秘密资源 ID、27/27 结果、rollback 状态和 blocker 分类；不得写“预计通过”。管理凭证持久化必须为 0，第二次执行重复资源必须为 0。
