...
超过 30 天自动隐藏所有具体价格、折扣、合计数字和 coupon CTA；当前官方来源不能证明数字当前性时也执行同一 `STALE_FALLBACK`，显示“查看当前结账页”。不能只换一个“可能过期”标签却继续展示数字。
...
不得用 redirect、local storage 或不可见参数重新添加 attribution。
