...
`.toolspan-dev/` 已被 gitignore。证据符合 `schemas/release-evidence.schema.json`，不包含本机绝对路径、password、OAuth code、PKCE verifier、access token、refresh token 或 Authorization header。

## Inspector 与固定 SDK 的证据边界

Harness 先对 packed local Server 真实运行官方 `@modelcontextprotocol/inspector@latest` v2 CLI：
...
该负向 smoke 不传 `--header`、token、client secret 或任何 stored auth。OAuth state 与 client config 路径指向隔离临时目录；预期结果必须为 JSON `auth_required`、exit code 3，且目录仍为空。这证明 Inspector 能到达 packed ToolSpan 的 OAuth 保护边界。
...
1. 官方 Inspector latest v2 CLI 无凭证 `tools/list` 必须返回 `auth_required` / exit 3，且不写 auth store；
2. 官方 Inspector 以 Authorization Code + PKCE 完成 full-scope loopback OAuth；
3. 官方 Inspector `tools/list` 必须一次返回 exact 27 Tool Contract；
4. 官方 Inspector 调用 `devspace_info` 并读取 fixture `README.txt`；
5. 官方 Inspector `apply_patch` dry run 不得改文件，授权 apply 必须修改并能通过 MCP readback 观察；
6. 官方 Inspector 通过 `npm` allowlisted runner 启动固定 job，并持续 `poll_job` 到 completed；
...
synthetic config 的唯一 `allowedRoots` 条目是：
...
Owner password 由 harness 在内存中生成，只通过 stdin 交给 packed `password:init`；磁盘只保存 bcrypt hash。OAuth consent 必须把 password 发送到 loopback form，但不会把它放进命令行、配置、日志或 evidence。启动 Server 时使用非秘密环境变量 allowlist，因此 `CloudFlareAPIKEY` 等外部 credential 不会被子进程继承。
...
- `E-CODEX-01 = PASS`：Codex CLI 0.149.0-alpha.4.1 通过临时 HTTPS Quick Tunnel 与 OAuth/DCR 连接 synthetic ToolSpan，真实完成 exact 27、read、`apply_patch`、job 与 readback；remote digest 改变、local fixture digest 不变，且 OAuth/MCP 配置、Tunnel、进程、workspace 与下载 binary 全部清理。
