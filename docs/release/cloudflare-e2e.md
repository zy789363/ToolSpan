# Cloudflare 真实 API E2E（安全 runner）

`scripts/e2e-cloudflare.mjs` 只面向固定测试边界：Zone `aiqushi.top`、首选 hostname `mcp.aiqushi.top`（被占用时只允许 session-derived `mcp-e2e-<session>.aiqushi.top`）、Tunnel 名称前缀 `toolspan-e2e-`。它不接受 Zone、hostname、Secret 或确认文本的命令行参数，也不会覆盖来源未知的 DNS/Tunnel。
...
runner 读取 gitignored 的 `.toolspan-dev/test-environment.json`，只把 manifest 中声明的环境变量名称用于当前 Node 进程内查值。Secret value 不进入参数、stdout/stderr、日志、Dry Run、evidence、Prompt、receipt 或诊断。
...
6. 生成带 hash 的 sanitized Dry Run。Plan hash 同时绑定 zone ID、account ID、实际 hostname、目标 Tunnel、当前只读 inspection fingerprint，以及 Tunnel `config_src`、exact ingress、DNS type/content rule/proxied/ttl 等完整 mutation payload。
...
在 PowerShell 中，确认 Owner 已审阅 Dry Run 后才可运行：
...
POST/PUT/DELETE 都不会自动重放。即使 HTTP 为 2xx，mutation response 缺少 exact config/ID、语义与请求不一致，或无法可靠解析，也按不确定结果处理：receipt 记录 `OUTCOME_UNKNOWN`，decision 为 `RECONCILE_REQUIRED`；后续只读 reconcile 不会凭名称采用资源，也不会盲目重试。

## 独立 Reconcile / second invocation
...
CLI 只接受固定格式的 session ID；receipt 路径始终由 runner 在 `.toolspan-dev/evidence` 内推导，不接受任意文件路径。Reconcile 加载并校验 schema v2 receipt、plan hash、固定 target、ID 和 fingerprint；Apply 时的 zone/plan/ownership identity 保持不可变，当前只读结果另存为 `reconcileZone` / `reconcile*Inspection`，临时 PENDING/缺失不会破坏后续恢复。second-run phase 只执行 GET，重新读取 Zone、DNS、Tunnel 与 ingress；Tunnel 必须仍是本 session 的 exact desired name，DNS 必须仍是指向该 Tunnel 的 exact proxied CNAME 且 TTL 为 1。只有这些语义全部匹配，且该 phase 的 mutation delta 为 `0`，`secondRun.status` 才能为 `PASS`。不请求 cleanup 时仍返回 `OWNED_CLEANUP_PENDING`；请求 cleanup 时也只会在 second-run PASS 和独立确认之后进入 DELETE phase。

如果 receipt 停在 mutation 前的 crash checkpoint，或记录 `OUTCOME_UNKNOWN`，Reconcile 仍会只读检查实际状态，但保持 fail-closed：不重放、不采用、不删除。
...
Apply 后必须由上述独立 Reconcile invocation 真实重新读取 Zone/DNS/Tunnel/config，要求 duplicate create 为 `0`。未清理时结果为 `OWNED_CLEANUP_PENDING`，不会虚构生命周期完成。

## Reconcile 中的 owned-only cleanup

cleanup 只能附加到独立的 Reconcile invocation：
...
每次运行在写文件前做两次结构化 Secret scan，输出到：

```text
.toolspan-dev/evidence/cloudflare-e2e-<session>.json
```

目录已由 `.gitignore` 排除。同一个固定路径既是当前脱敏 evidence，也是 crash-safe receipt；写入采用同目录临时文件后原子 rename。它只包含固定目标、非秘密 ID/fingerprint、status、nameserver、资源类型/所有权分类、planned/applied action、checkpoint、HTTP status/Cloudflare 数字错误码以及 second-run/cleanup 结果；不包含 header、请求/响应 body、Cloudflare message、email 或任意 Secret value。格式由 `schemas/cloudflare-e2e-evidence.schema.json` v2 固定。

这里的 schema 校验和 atomic rename 只防格式漂移、半写文件与 runner 自身误操作，不是签名，也不构成对同一 Windows 用户、本地管理员、junction/reparse point 或并发进程的防篡改保证。`.toolspan-dev/evidence` 必须保持为 Owner 控制的普通本地目录；同一 session 不得并发运行 Apply/Reconcile。发现 receipt/schema/identity 不一致、cleanup 非终态或 mutation response 不确定时，CLI 一律 fail-closed，不继续 cleanup。

CLI exit code 不混淆成功、外部阻塞与人工 checkpoint：
